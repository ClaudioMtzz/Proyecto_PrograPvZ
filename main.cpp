...
public:
    ...
    Proyectil(sf::Texture &textura, float x, float y, bool hielo) {
        ...
        borrar = false;
        esHielo = hielo;
        if (esHielo) shape.setColor(sf::Color::Cyan); 
    }
...
public:
    ...
    Planta(sf::Texture &textura, float x, float y, int tipoPlanta) {
        ...
        if (tipo == 1) { // LANZAGUISANTES
            shape.setScale(0.18f, 0.18f); vida = 5;  
        } else if (tipo == 2) { // NUEZ
            shape.setScale(0.08f, 0.08f); vida = 30; 
        } else if (tipo == 3) { // GIRASOL
            shape.setScale(0.15f, 0.15f); vida = 5; 
        } else if (tipo == 4) { // HIELAGUISANTES
            shape.setScale(0.18f, 0.18f); vida = 6;
    ...
    void recargar() { relojDisparo.restart(); }

    int producirSoles() {
        if (tipo == 3) { 
            if (relojProduccion.getElapsedTime().asSeconds() > 5.0f) { 
                relojProduccion.restart(); return 25; 
            }
        }
        return 0; 
    }
...
public:
    ...
    bool update(std::vector<Planta> &jardin, sf::Sound &sonidoComer) {
        if (congelado && relojCongelado.getElapsedTime().asSeconds() > 2.0f) {
            congelado = false; velocidadActual = velocidadBase; 
        ...
        if (!estaComiendo) shape.move(-velocidadActual, 0); 
...
// --- MAIN ---
int main() {
    sf::RenderWindow window(sf::VideoMode(800, 600), "PvZ - Fase 16: DIFICULTAD PROGRESIVA");
    ...
    // Rutas con "assets/"
    texFondo.loadFromFile("assets/textures/fondo.png"); 
    if (!texZombie.loadFromFile("assets/textures/zombie.png")) return -1;
    if (!texPlanta.loadFromFile("assets/textures/planta.png")) return -1;
    if (!texNuez.loadFromFile("assets/textures/nuez.png")) return -1; 
    if (!texGirasol.loadFromFile("assets/textures/girasol.png")) return -1;
    if (!texBala.loadFromFile("assets/textures/bala.png")) return -1;
    
    if (!texHiela.loadFromFile("assets/textures/hiela.png")) texHiela = texPlanta; 
    if (!texFutbol.loadFromFile("assets/textures/futbol.png")) texFutbol = texZombie; 
    if (!texBalaHielo.loadFromFile("assets/textures/hielo.png")) texBalaHielo = texBala;
    if (!texPala.loadFromFile("assets/textures/pala.png")) texPala = texZombie; 

    sf::Font fuente;
    if (!fuente.loadFromFile("assets/arial.ttf")) return -1;

    // AUDIO
    sf::Music musicaFondo;
    if (musicaFondo.openFromFile("assets/sounds/musica.ogg")) {
    ...
    if (bufferPiu.loadFromFile("assets/sounds/piu.wav")) sonidoPiu.setBuffer(bufferPiu);
    if (bufferGolpe.loadFromFile("assets/sounds/golpe.wav")) sonidoGolpe.setBuffer(bufferGolpe);
    if (bufferComer.loadFromFile("assets/sounds/comer.wav")) { sonidoComer.setBuffer(bufferComer); sonidoComer.setVolume(80); }
    if (bufferGameOver.loadFromFile("assets/sounds/gameover.wav")) { sonidoGameOver.setBuffer(bufferGameOver); sonidoGameOver.setVolume(100); }
    ...
    // Texto de "Nivel/Tiempo" para que el jugador sepa que se pone dificil
    sf::Text textoTiempo;
    textoTiempo.setFont(fuente); textoTiempo.setCharacterSize(20); textoTiempo.setFillColor(sf::Color::Black); textoTiempo.setPosition(600, 10);
    ...
    sf::RectangleShape btnPala(sf::Vector2f(60, 60)); btnPala.setPosition(430, 10); btnPala.setFillColor(sf::Color(100, 100, 100)); 
    sf::Sprite iconoPala(texPala); iconoPala.setPosition(435, 15); iconoPala.setScale(0.1f, 0.1f); 
    ...
    // !!! RELOJ DE DIFICULTAD GLOBAL !!!
    sf::Clock relojGlobalJuego;
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (juegoTerminado && event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::R) {
                ...
                relojGlobalJuego.restart(); // Reiniciar dificultad
            ...
            if (!juegoTerminado && event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (btnGuisante.getGlobalBounds().contains(mx, my)) plantaSeleccionada = 1;
                    ...
                    else if (btnPala.getGlobalBounds().contains(mx, my)) plantaSeleccionada = 0; 
                        ...
                        if (plantaSeleccionada == 0) { 
                            for(auto it = jardin.begin(); it != jardin.end(); ++it) {
                                ...
                                if(pCol == col && pRow == row) { jardin.erase(it); break; }
                            }
                        } else { 
                            int costo = 50; 
        ...
        if (!juegoTerminado) {
            ...
            btnPala.setOutlineThickness(plantaSeleccionada == 0 ? 3 : 0); 

            // ECONOMIA
            if (relojIngresoPasivo.getElapsedTime().asSeconds() > 10.0f) { 
                dineroSoles += 10; relojIngresoPasivo.restart();
            }
            for (auto &planta : jardin) if(planta.producirSoles() > 0) dineroSoles += 25;
            textoSoles.setString("Soles: " + std::to_string(dineroSoles));
            
            // --- LOGICA DE DIFICULTAD PROGRESIVA ---
            float tiempoJuego = relojGlobalJuego.getElapsedTime().asSeconds();
            float tiempoEntreSpawn = 5.0f; // Default (Facil)
            int probabilidadCubeta = 0;
            int probabilidadFutbol = 0;

            if (tiempoJuego < 40.0f) {
                // FASE 1: CALENTAMIENTO (0-40s)
                tiempoEntreSpawn = 5.0f;
                probabilidadCubeta = 10; // 10%
                probabilidadFutbol = 0;  // 0%
                textoTiempo.setString("Nivel: Facil");
            } else if (tiempoJuego < 90.0f) {
                // FASE 2: MEDIA (40-90s)
                tiempoEntreSpawn = 3.5f; // Mas rapido
                probabilidadCubeta = 30; // 30%
                probabilidadFutbol = 10; // 10%
                textoTiempo.setString("Nivel: Medio");
            } else {
                // FASE 3: DIFICIL (90s+)
                tiempoEntreSpawn = 2.0f; // Muy rapido!
                probabilidadCubeta = 40; 
                probabilidadFutbol = 30; // 30% Futbolistas
                textoTiempo.setString("Nivel: DIFICIL!");
                textoTiempo.setFillColor(sf::Color::Red);
            }

            // GENERADOR ZOMBIES (Usando variables dinamicas)
            if (relojGeneradorZombies.getElapsedTime().asSeconds() > tiempoEntreSpawn) {
                ...
                // Logica de porcentajes
                if (r < probabilidadFutbol) tipoZombie = 3; // Prioridad al futbolista
                else if (r < (probabilidadFutbol + probabilidadCubeta)) tipoZombie = 2;
        ...
        window.draw(textoTiempo); // Mostrar Nivel
